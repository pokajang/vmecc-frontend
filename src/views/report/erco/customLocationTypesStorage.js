...
const ERCO_CUSTOM_LOCATION_TYPE_KEY = 'report_erco_custom_location_types_v1_user_'
...
export const loadCustomLocationTypes = (userId) => {
  if (!userId) return []
  const rows = parse(localStorage.getItem(storageKey(ERCO_CUSTOM_LOCATION_TYPE_KEY, userId)), [])
...
export const saveCustomLocationTypes = (userId, rows) => {
  ...
  localStorage.setItem(
    storageKey(ERCO_CUSTOM_LOCATION_TYPE_KEY, userId),
