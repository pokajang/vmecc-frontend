...
const ERCO_CUSTOM_WEATHER_TYPE_KEY = 'report_erco_custom_weather_types_v1_user_'
...
export const loadCustomWeatherTypes = (userId) => {
  if (!userId) return []
  const rows = parse(localStorage.getItem(storageKey(ERCO_CUSTOM_WEATHER_TYPE_KEY, userId)), [])
...
export const saveCustomWeatherTypes = (userId, rows) => {
  if (!userId) return
  const normalizedRows = Array.isArray(rows) ? rows.map(normalizeCustomType).filter(Boolean) : []
  localStorage.setItem(
    storageKey(ERCO_CUSTOM_WEATHER_TYPE_KEY, userId),
