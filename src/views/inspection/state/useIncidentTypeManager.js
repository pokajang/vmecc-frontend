...
const useIncidentTypeManager = ({ userId, selectedType, updateSetupField, pushToast }) => {
  ...
  const incidentSystemOptions = useMemo(
    () => applyTypeOverrides(INSPECTION_INCIDENT_TYPE_OPTIONS, incidentSystemOverrides),
  ...
  const systemTypeSet = useMemo(
    () =>
      new Set(INSPECTION_INCIDENT_TYPE_OPTIONS.map((row) => String(row.value || '').toLowerCase())),
    [],
  )
  ...
  const saveType = () => {
    ...
    if (editKey && systemTypeSet.has(editKey)) {
      const baseOption = INSPECTION_INCIDENT_TYPE_OPTIONS.find(
        (row) =>
          String(row.value || '')
            .trim()
            .toLowerCase() === editKey,
