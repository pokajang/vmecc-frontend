...
import { INSPECTION_LOCATION_OPTIONS } from './constants'
...
const useLocationTypeManager = ({ userId, selectedLocations, updateSetupField, pushToast }) => {
  ...
  const locationSystemOptions = useMemo(
    () => applyTypeOverrides(INSPECTION_LOCATION_OPTIONS, locationSystemOverrides),
    ...
    () =>
      buildPinnedVisibleOptions({
        ...
        toggleOption: {
          ...
          description: showAllLocationTypes ? 'Hide extra locations.' : 'View all locations.',
  ...
  const systemTypeSet = useMemo(
    () => new Set(INSPECTION_LOCATION_OPTIONS.map((row) => String(row.value || '').toLowerCase())),
    [],
  )
  ...
  const saveType = () => {
    ...
    if (exists) {
      setAddLocationError('This location already exists.')
      return
    }

    if (editKey && systemTypeSet.has(editKey)) {
      const baseOption = INSPECTION_LOCATION_OPTIONS.find(
        (row) =>
          String(row.value || '')
            .trim()
            .toLowerCase() === editKey,
      ...
      if (typeof pushToast === 'function') {
        pushToast(`Location "${title}" updated.`, {
    ...
    if (typeof pushToast === 'function') {
      pushToast(editKey ? `Location "${title}" updated.` : `Location "${title}" added.`, {
  ...
  const resetSystemOverride = (value) => {
    ...
    if (typeof pushToast === 'function') {
      pushToast('Location reset to default.', {
