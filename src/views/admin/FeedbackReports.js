...
const FeedbackReports = () => {
  ...
  const loadReports = useCallback(async () => {
    ...
    setError(null)
    try {
      const response = await fetchFeedbackReports({ status, per_page: 50 })
      setReports(response?.data || [])
      setCounts(response?.meta?.counts || {})
    } catch (err) {
      setError(err?.payload?.message || err?.message || 'Unable to load feedback reports.')
    } finally {
      setLoading(false)
    }
  }, [canReview, status])
  ...
  const openDetail = useCallback(async (reportId) => {
    ...
    setDetailError(null)
    try {
      const response = await fetchFeedbackReport(reportId)
      const detail = response?.data || null
      setSelected(detail)
      setFormStatus(detail?.status || 'new')
      setAdminNote(detail?.admin_note || '')
    } catch (err) {
      setDetailError(err?.payload?.message || err?.message || 'Unable to load feedback report.')
    } finally {
  ...
  const saveDetail = useCallback(async () => {
    ...
    setDetailError(null)
    try {
      const response = await updateFeedbackReport(selected.id, {
        status: formStatus,
      ...
      setDetailError(err?.payload?.message || err?.message || 'Unable to update feedback report.')
    } finally {
      setSaving(false)
    }
  ...
  return (
    <CContainer fluid data-testid="feedback-reports-module">
      ...
      >
        <CModalHeader>
          <CModalTitle>Feedback report</CModalTitle>
        </CModalHeader>
        <CModalBody>
          {detailLoading ? (
            ...
            <>
              ...
              <CRow className="g-3 mb-3">
                ...
                <CCol md={6}>
                  ...
                  <div>
                    {selected.reviewed_at ? formatDateTime(selected.reviewed_at) : 'Not reviewed'}
                  </div>
                </CCol>
              </CRow>
              ...
              <CRow className="g-3 mb-3">
                <CCol md={6}>
                  ...
                  <div className="text-break">{selected.reporter_ip || 'Not available'}</div>
                </CCol>
                <CCol md={6}>
                  <div className="text-muted small">User agent</div>
                  <div className="text-break">
                    {selected.reporter_user_agent || 'Not available'}
                  </div>
                </CCol>
              </CRow>
              ...
              <CRow className="g-3">
                <CCol md={4}>
                  <CFormLabel htmlFor="feedback-report-status">Status</CFormLabel>
                  <CFormSelect
                    id="feedback-report-status"
                    value={formStatus}
                ...
                <CCol md={8}>
                  <CFormLabel htmlFor="feedback-report-admin-note">Admin note</CFormLabel>
                  <CFormTextarea
                    id="feedback-report-admin-note"
                    value={adminNote}
