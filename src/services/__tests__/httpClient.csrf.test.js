...
describe('httpClient CSRF handling', () => {
  ...
  it('sends the csrf token on unsafe API requests', async () => {
    ...
    expect(fetch.mock.calls[0][1].headers['X-Client-Mode']).toBe('browser')
  })

  it('detects PWA display mode and sends it as a request header', async () => {
    vi.stubGlobal(
      'matchMedia',
  ...
  it('fetches session token before unsafe requests when missing', async () => {
    fetch
      .mockResolvedValueOnce(jsonResponse({ csrf_token: 'refreshed-token', user: { id: 1 } }))
      .mockResolvedValueOnce(jsonResponse({ ok: true }))

    await apiRequest('/profile', {
      method: 'PUT',
      body: JSON.stringify({ name: 'Refreshed' }),
    })
